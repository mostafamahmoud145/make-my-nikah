...
class StartCall {
  ...
  Future startCall() async {
    ...
    if (appointment?.consultType != 'video') {
      // Forcing the video-mute.enabled to false
      featureFlags['video-mute.enabled'] = false;
    }
  ...
  _hangUp() {
    if (isHangupBefore == false) {
      ...
      if (!isUserCanceled) {
        ...
        if (loggedUser?.userType == AppConstants.coach &&
            anotherSideUser?.userType == AppConstants.consultant) {
          confirmEndCallDialog(
              context: context,
              loggedUser: loggedUser!,
              appointment: appointment!);
        } else if (anotherSideUser?.userType == AppConstants.coach &&
            loggedUser?.userType == AppConstants.consultant) {
          // DO nothing
        } else if (loggedUser!.userType == AppConstants.coach ||
            loggedUser!.userType == AppConstants.consultant) {
          confirmEndCallDialog(
              context: context,
              loggedUser: loggedUser!,
  ...
  void checkCallerState() async {
    'in CheckCallerState Function'.logPrint();
