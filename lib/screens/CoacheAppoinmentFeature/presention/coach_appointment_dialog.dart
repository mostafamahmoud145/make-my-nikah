...
import 'package:grocery_store/screens/CoacheAppoinmentFeature/presention/widget/appointmentDialogwidget/invoice_widget.dart';
...
import 'package:grocery_store/screens/CoacheAppoinmentFeature/utils/service/Funcation/check_balance.dart';
...
import 'dart:io';
import 'widget/appointmentDialogwidget/order_details_widget.dart';
import '../../../widget/payment_radio_button.dart';
import 'widget/appointmentDialogwidget/text_dialog.dart';

class CoachAppointment extends StatefulWidget {
  ...
  @override
  _CoachAppointmentState createState() => _CoachAppointmentState();
}

class _CoachAppointmentState extends State<CoachAppointment> {
  ...
  bool hijri = false,
      ...
      dateSelected = true,
      dateUnSelect = false;
  String time = DateFormat('d/M/yyyy').format(DateTime.now()), dateText = "";
  String?
    ...
    return AvailableDays(
        ...
        onTapHijri: () {
          setState(() {
            ...
            time = DateFormat('d/M/yyyy').format(DateTime.now());
            gregorian = false;
            hijri = true;
          });
        },
        onTapGregorian: () {
          setState(() {
            displayedTime = DateFormat('d/M/yyyy').format(DateTime.now());
            selectedDate = DateTime.now();
            time = DateFormat('d/M/yyyy').format(DateTime.now());
            gregorian = true;
            hijri = false;
  ...
  Widget availableHours() {
    ...
    // coachAppointmentCubit.date = DateTime.parse(selectedTime??(todayAppointmentList.isNotEmpty?todayAppointmentList.first:)).toLocal();
    coachAppointmentCubit.selectedCard = selectedCard;
    coachAppointmentCubit.time = time;
    coachAppointmentCubit.todayAppointmentList = todayAppointmentList;
    return AvailableHourss(
        ...
        onChanged: (value) {
          ...
          coachAppointmentCubit.date = DateTime.parse(value).toLocal();
          coachAppointmentCubit.selectedCard = selectedCard;
          coachAppointmentCubit.time = time;
          coachAppointmentCubit.todayAppointmentList = todayAppointmentList;
      ...
      if (codes.length > 0) {
        ...
        if ((codes[0].type == "default" && codes[0].promoCodeStatus) ||
          ...
          setState(() {
            ...
            BlocProvider.of<CoachAppointmentCubit>(context).discountt =
                promo!.discount;
          });
        else
          setState(() {
  ...
  Future<void> _selectDate(BuildContext context) async {
    try {
      ...
      if (picked != null && picked != selectedDate) {
        setState(() {
          selectedDate = picked;
          time = DateFormat('d/M/yyyy').format(picked);
          displayedTime = time;
          loadDates = true;
          todayAppointmentList = [];
  ...
  Future<Null> _selectHijriDate(BuildContext context) async {
    try {
      var currentHijriDate=new HijriCalendar.now();
      final HijriCalendar? picked = await showHijriDatePicker(
        context: context,
        initialDate: currentHijriDate,
        lastDate: new HijriCalendar()
          ..hYear = currentHijriDate.hYear+1
          ..hMonth = currentHijriDate.hMonth
          ..hDay = currentHijriDate.hDay,
        firstDate: currentHijriDate,
       /*  new HijriCalendar()
          ..hYear = 1438
          ..hMonth = 12
          ..hDay = 25, */
      ...
      if (picked != null) {
        setState(() {
          ...
          time = DateFormat('d/M/yyyy').format(selectedDate);
      ...
      print("_selectHijriDateerror"+e.toString());
    }
  }
}
